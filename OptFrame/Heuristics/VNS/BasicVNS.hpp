...
class BasicVNS: public VariableNeighborhoodSearch<R, ADS, DS>
{
public:

	typedef VariableNeighborhoodSearch<R, ADS, DS> super;

	BasicVNS(Evaluator<R, ADS, DS>& evaluator, Constructive<R, ADS>& constructive, vector<NS<R, ADS, DS>*> vshake, vector<NSSeq<R, ADS, DS>*> vsearch) :
	...
	{
		return * new BestImprovement<R, ADS, DS>(super::evaluator, *super::vsearch.at(k_search));
	}
	...
	{
		stringstream ss;
		ss << VariableNeighborhoodSearch<R, ADS, DS >::idComponent() << "BVNS";
		return ss.str();
	}
};

template<class R, class ADS = OPTFRAME_DEFAULT_ADS, class DS = OPTFRAME_DEFAULT_DS>
class BasicVNSBuilder : public ILS, public SingleObjSearchBuilder<R, ADS, DS>
{
public:
	...
	{
		stringstream ss;
		ss << SingleObjSearchBuilder<R, ADS, DS>::idComponent() << VNS::family() << "BasicVNS";
		return ss.str();
	}
