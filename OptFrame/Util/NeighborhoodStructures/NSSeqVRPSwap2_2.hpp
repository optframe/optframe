...
class MoveVRPSwap2_2: public Move<vector<vector<T> > >
{
...
public:
	...
	MoveVRPSwap2_2(int _r1, int _r2, int _c1, int _c2, bool _reverseR1, bool _reverseR2) :
		r1(_r1), r2(_r2), c1(_c1), c2(_c2), reverseR1(_reverseR1), reverseR2(_reverseR2)
	{
	}
	...
	virtual MoveVRPSwap2_2<T, ADS, M>& apply(Routes& rep)
	{
		int aux;
		...
		return *new MoveVRPSwap2_2<T, ADS, M> (r2, r1, c2, c1, reverseR1, reverseR2);
	}

	virtual bool operator==(const Move<Routes>& _m) const
	{
		const MoveVRPSwap2_2<T, ADS, M>& m1 = (const MoveVRPSwap2_2<T, ADS, M>&) _m;
...
private:
	...
	MoveVRPSwap2_2<T, ADS, M>* m;
	vector<MoveVRPSwap2_2<T, ADS, M>*> moves;
...
public:
				...
				for (int r2 = r1 + 1; r2 < r.size(); r2++)
				{
					if (r.at(r2).size() >= 2) //verificando se há pelo menos dois clientes.
					{
						for (int c1 = 0; c1 < r.at(r1).size() - 1; c1++)
						{
							...
							{
								moves.push_back(new MoveVRPSwap2_2<T, ADS, M> (r1, r2, c1, c2, false, false));//normal
								moves.push_back(new MoveVRPSwap2_2<T, ADS, M> (r1, r2, c1, c2, true, false));//reverse route 1
								moves.push_back(new MoveVRPSwap2_2<T, ADS, M> (r1, r2, c1, c2, false, true));//reverse route 2
								moves.push_back(new MoveVRPSwap2_2<T, ADS, M> (r1, r2, c1, c2, true, true));//reverse booth
							}
						}
					}
				}
	...
	MoveVRPSwap2_2<T, ADS, M> & current()
	{
		if (isDone())
		{
...
public:
	...
	MoveVRPSwap2_2<T, ADS, M>& move(const Routes& rep)
	{
		if (rep.size() < 2)
			return *new MoveVRPSwap2_2<T, ADS, M> (-1, -1, -1, -1, false, false);
		...
		return *new MoveVRPSwap2_2<T, ADS, M> (r1, r2, c1, c2, reverseR1, reverseR2);
	}

	virtual NSIteratorVRPSwap2_2<T, ADS, M>& getIterator(const Routes& r)
	{
		return *new NSIteratorVRPSwap2_2<T, ADS, M> (r);
	}

	virtual void print()
	{
		cout << "NSSeqVRPSwap2_2" << endl;
	}
};
