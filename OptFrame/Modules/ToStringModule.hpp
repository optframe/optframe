...
#ifndef TOSTRING_MODULE_HPP_
#define TOSTRING_MODULE_HPP_
...
class ToStringModule: public OptFrameModule<R, ADS, M>
{
public:
   ...
   */
   ...
   string usage()
   {
      return "to_string component [store_str]";
   }
   ...
   */
   ...
   bool run(vector<OptFrameModule<R, ADS, M>*>& all_modules, vector<OptFrameFunction*>& allFunctions, HeuristicFactory<R, ADS, M>& factory, map<string, string>& dictionary, map< string,vector<string> >& ldictionary, string input)
   {
	   ...
	   else
	   {
		   ...
		   return OptFrameModule<R, ADS, M>::run_module("silent_define", all_modules, allFunctions, factory, dictionary, ldictionary, sstr.str());
	   }
   }

};

#endif /* TOSTRING_MODULE_HPP_ */
