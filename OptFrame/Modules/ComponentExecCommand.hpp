...
class ComponentExecCommand: public Command<R, ADS, DS>
{
public:

	virtual ~ComponentExecCommand()
	{
	}
	...
	bool run(vector<Command<R, ADS, DS>*>& all_modules, vector<PreprocessFunction<R, ADS, DS>*>& allFunctions, HeuristicFactory<R, ADS, DS>& factory, map<string, string>& dictionary,  map< string,vector<string> >& ldictionary, string input)
	{
		...
		if (scanner.hasNext())
		{
			...
			if(!Command<R, ADS, DS>::run_module("system.silent_define", all_modules, allFunctions, factory, dictionary, ldictionary, new_name + " " + s_new_id))
				return false;
		}
		...
		if (scanner.hasNext())
		{
			...
			stringstream ss;
			ss.precision(Command<R, ADS, DS>::precision);
			ss << fixed;
			ss << var_time << " " << time;
			if(!Command<R, ADS, DS>::run_module("system.silent_define", all_modules, allFunctions, factory, dictionary, ldictionary, ss.str()))
				return false;
		}
