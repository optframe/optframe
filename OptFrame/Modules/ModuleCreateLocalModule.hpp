...
class LocalModule: public Module<R, ADS, DS>
{
	string name;
...
public:
	LocalModule(string _name, vector<string>& _parameters, vector<string>& _commands)
	{
		name = _name;
...
public:
	...
	bool run(vector<Module<R, ADS, DS>*>& all_modules, vector<PreprocessFunction<R, ADS, DS>*>& allFunctions, HeuristicFactory<R, ADS, DS>& factory, map<string, string>& dictionary, map<string, vector<string> >& ldictionary, string input)
	{
		...
		if (!Module<R, ADS, DS>::run_module("system.run", all_modules, allFunctions, factory, dictionary1, ldictionary1, OptFrameList::blockToString(commands)))
		{
		...
		for (unsigned p = 0; p < parameters.size(); p++)
			Module<R, ADS, DS>::undefine(cleanReference(parameters[p]), dictionary, ldictionary);
...
template<class R, class ADS = OPTFRAME_DEFAULT_ADS, class DS = OPTFRAME_DEFAULT_DS>
class ModuleCreateLocalModule: public Module<R, ADS, DS>
{
private:
...
public:
	...
	virtual ~ModuleCreateLocalModule()
	{
	}
	...
	bool run(vector<Module<R, ADS, DS>*>& modules, vector<PreprocessFunction<R, ADS, DS>*>& allFunctions, HeuristicFactory<R, ADS, DS>& factory, map<string, string>& dictionary, map<string, vector<string> >& ldictionary, string input)
	{
		...
		{
			modules.push_back(new LocalModule<R, ADS, DS>(name, parameters, commands));
