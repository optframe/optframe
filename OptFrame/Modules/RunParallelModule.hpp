...
class RunParallelModule: public OptFrameModule<R, ADS, M>
{
private:
	// INPUT
...
public:
	...
	bool run(vector<OptFrameModule<R, ADS, M>*>& _allModules, vector<OptFrameFunction*>& _allFunctions, HeuristicFactory<R, ADS, M>& _factory, map<string, string>& _dictionary, string input)
	{
		...
		for(unsigned t=0; t<allCommands.size(); t++)
		{
	        ...
	        pthread_create(&threads[t], 0, &RunParallelModule<R, ADS, M>::start_thread, this);
		}
	...
	{
		reinterpret_cast<RunParallelModule<R, ADS, M> *>(obj)->exec_command();
		pthread_exit(NULL);
	}
