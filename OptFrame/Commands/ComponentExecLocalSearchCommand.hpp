...
class ComponentExecLocalSearchCommand: public Command<R, ADS, DS>
{
public:

	virtual ~ComponentExecLocalSearchCommand()
	{
	}
	...
	bool run(vector<Command<R, ADS, DS>*>& all_modules, vector<PreprocessFunction<R, ADS, DS>*>& allFunctions, HeuristicFactory<R, ADS, DS>& factory, map<string, string>& dictionary,  map< string,vector<string> >& ldictionary, string input)
	{
		...
		if (scanner.hasNext())
		{
			...
			if(!Command<R, ADS, DS>::defineText(new_name, s_new_id, dictionary))
				return false;
		}
		...
		if (scanner.hasNext())
		{
			...
			stringstream sstime;
			sstime.precision(Command<R, ADS, DS>::precision);
			sstime << fixed;
			sstime << time;
			if(!Command<R, ADS, DS>::defineText(var_time, sstime.str(), dictionary))
				return false;
		}
