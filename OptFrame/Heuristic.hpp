...
public:
	...
	// no-optimization
	Solution<R>& search(const Solution<R>& s, double timelimit = 100000000,
			double target_f = 0)
	{
	...
	// optimizated version
	pair<Solution<R>&, Evaluation<M>&>& search(const Solution<R>& s,
			const Evaluation<M>& e, double timelimit = 100000000,
			double target_f = 0)
	{
		Solution<R>& s2 = s.clone();
	...
	// no-optimization
	Population& search(ConstPopulation& p, double timelimit = 100000000,
			double target_f = 0)
	{
		Population* v = new Population;
		for (unsigned i = 0; i < p.size(); i++)
			v->push_back(&p[i]->clone());
	...
	// optimizated version
	pair<Population&, FitnessValues&>& search(ConstPopulation& p,
			ConstFitnessValues& ev, double timelimit = 100000000,
			double target_f = 0)
	{
		...
		return *new pair<Population&, FitnessValues&> (*p2, *ev2);
	}
	...
	// 4
	virtual void exec(Population& p, FitnessValues& ev, double timelimit,
			double target_f)
	{
		// call 2
		Solution<R>& s = *(p[0]);
