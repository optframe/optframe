...
#include <OptFrame/boost/config/workaround.hpp>
...
struct is_string_like<
    T, CharT,
    void_t<decltype(std::declval<CharT const*&>() = std::declval<T>().data()),
...
struct enable_if_viewable<
    ...
    typename std::enable_if<
        ...
        !std::is_convertible<
            const T&, const basic_static_string<N, CharT, Traits>&>::value
        ...
        (std::is_convertible<const T&,
         ...
         std::is_convertible<const T&,
                             core::basic_string_view<CharT>>::value) &&
        !std::is_convertible<const T&, const CharT*>::value &&
        !std::is_convertible<
            const T&, const basic_static_string<N, CharT, Traits>&>::value
#endif
...
using enable_if_viewable_t =
    typename enable_if_viewable<N, T, CharT, Traits>::type;

// The common string_view type used in private operations with
// enable_if_viewable_t
...
struct common_string_view_type_impl<
    ...
    typename std::enable_if<
        ...
        !std::is_convertible<const T&,
                             basic_string_view<CharT, Traits>>::value &&
        !std::is_convertible<const T&,
                             core::basic_string_view<CharT>>::value>::type> {
  using type = T const&;
};
...
struct common_string_view_type_impl<
    ...
    typename std::enable_if<std::is_convertible<
        const T&, core::basic_string_view<CharT>>::value>::type> {
  using type = core::basic_string_view<CharT>;
};

template <typename T, typename CharT, typename Traits>
using common_string_view_type =
    typename common_string_view_type_impl<T, CharT, Traits>::type;
#endif
...
template <typename ForwardIt,
          typename std::enable_if<!is_subtractable<ForwardIt>::value>::type* =
              nullptr>
BOOST_STATIC_STRING_CPP14_CONSTEXPR std::size_t distance(ForwardIt first,
                                                         ForwardIt last) {
  std::size_t dist = 0;
...
BOOST_STATIC_STRING_CPP14_CONSTEXPR std::size_t distance(RandomIt first,
                                                         RandomIt last) {
  return last - first;
}
...
BOOST_STATIC_STRING_CPP14_CONSTEXPR void copy_with_traits(InputIt first,
                                                          InputIt last,
                                                          CharT* out) {
  for (; first != last; ++first, ++out) Traits::assign(*out, *first);
}
...
template <typename CharT, typename Traits>
BOOST_STATIC_STRING_CPP14_CONSTEXPR inline int lexicographical_compare(
    const CharT* s1, std::size_t n1, const CharT* s2, std::size_t n2) noexcept {
  if (n1 < n2) return Traits::compare(s1, s2, n1) <= 0 ? -1 : 1;
  if (n1 > n2) return Traits::compare(s1, s2, n2) >= 0 ? 1 : -1;
...
template <std::size_t N>
inline static_string<N> to_static_string_float_impl(
...
template <std::size_t N>
inline static_wstring<N> to_static_wstring_float_impl(
    long double value) noexcept {
  // we have to assume here that no reasonable implementation
  // will require more than 2^63 chars to represent a float value.
  const long long narrow = static_cast<long long>(N);
...
BOOST_STATIC_STRING_CPP14_CONSTEXPR inline ForwardIterator find_not_of(
    ForwardIterator first, ForwardIterator last, const CharT* str,
    std::size_t n) noexcept {
  for (; first != last; ++first)
    if (!Traits::find(str, n, *first)) return first;
  return last;
}

// constexpr search for C++14
template <typename ForwardIt1, typename ForwardIt2, typename BinaryPredicate>
BOOST_STATIC_STRING_CPP14_CONSTEXPR inline ForwardIt1 search(
...
BOOST_STATIC_STRING_CPP14_CONSTEXPR inline InputIt find_first_of(
...
template <typename T>
BOOST_STATIC_STRING_CPP14_CONSTEXPR inline bool ptr_in_range(const T* src_first,
                                                             const T* src_last,
                                                             const T* ptr) {
...
#endif
{
...
#endif
            >
  BOOST_STATIC_STRING_CPP14_CONSTEXPR basic_static_string(InputIterator first,
                                                          InputIterator last) {
...
#endif
            >
  BOOST_STATIC_STRING_CPP14_CONSTEXPR basic_static_string(const T& t,
                                                          size_type pos,
                                                          size_type n) {
    assign(t, pos, n);
  }
  ...
  */
  BOOST_STATIC_STRING_CPP14_CONSTEXPR
  basic_static_string& operator=(const basic_static_string& s) {
    return assign(s);
  }
...
#endif
            >
  BOOST_STATIC_STRING_CPP14_CONSTEXPR basic_static_string& assign(
      const basic_static_string<M, CharT, Traits>& s) {
  ...
  */
  template <std::size_t M>
  BOOST_STATIC_STRING_CPP14_CONSTEXPR basic_static_string& assign(
      const basic_static_string<M, CharT, Traits>& s, size_type pos,
  ...
  */
  BOOST_STATIC_STRING_CPP14_CONSTEXPR
  basic_static_string& assign(const_pointer s) {
  ...
  */
  ...
  BOOST_STATIC_STRING_CPP14_CONSTEXPR basic_static_string& insert(
      size_type index, const basic_static_string<M, CharT, Traits>& str) {
  ...
  */
  template <typename T
            ...
            >
  BOOST_STATIC_STRING_CPP14_CONSTEXPR basic_static_string& insert(
      size_type index, const T& t, size_type index_str,
      size_type count = npos) {
    detail::common_string_view_type<T, CharT, Traits> sv(t);
    if (index_str > sv.size())
      detail::throw_exception<std::out_of_range>("index_str > t.size()");
    return insert(index, sv.data() + index_str,
                  (std::min)(sv.size() - index_str, count));
  }
  ...
  */
  template <std::size_t M>
  BOOST_STATIC_STRING_CPP14_CONSTEXPR basic_static_string& append(
      const basic_static_string<M, CharT, Traits>& s) {
  ...
  */
  BOOST_STATIC_STRING_CPP14_CONSTEXPR
  basic_static_string& append(const_pointer s) {
  ...
  */
  template <typename T
            ...
            >
  BOOST_STATIC_STRING_CPP14_CONSTEXPR basic_static_string& append(
      const T& t, size_type pos, size_type count = npos) {
    detail::common_string_view_type<T, CharT, Traits> sv = t;
    if (pos > sv.size())
      detail::throw_exception<std::out_of_range>("pos > t.size()");
...
#endif
            >
  BOOST_STATIC_STRING_CPP14_CONSTEXPR int compare(size_type pos1,
                                                  size_type count1,
                                                  const T& t) const {
    detail::common_string_view_type<T, CharT, Traits> sv = t;
    return detail::lexicographical_compare<CharT, Traits>(
  ...
  */
  template <typename T
            ...
            >
  BOOST_STATIC_STRING_CPP14_CONSTEXPR int compare(
      size_type pos1, size_type count1, const T& t, size_type pos2,
      size_type count2 = npos) const {
    detail::common_string_view_type<T, CharT, Traits> sv = t;
    if (pos2 > sv.size())
      detail::throw_exception<std::out_of_range>("pos2 > sv.size()");
    return compare(pos1, count1, sv.data() + pos2,
  ...
  */
  template <std::size_t M>
  BOOST_STATIC_STRING_CPP14_CONSTEXPR void swap(
...
#ifndef BOOST_STATIC_STRING_DOCS
  BOOST_STATIC_STRING_CPP14_CONSTEXPR
  basic_static_string& replace(size_type pos1, size_type n1,
  ...
  */
  template <typename T
            ...
            >
  BOOST_STATIC_STRING_CPP14_CONSTEXPR basic_static_string& replace(
      size_type pos1, size_type n1, const T& t) {
    detail::common_string_view_type<T, CharT, Traits> sv = t;
  ...
  */
  template <typename T
            ...
            >
  BOOST_STATIC_STRING_CPP14_CONSTEXPR basic_static_string& replace(
      size_type pos1, size_type n1, const T& t, size_type pos2,
      size_type n2 = npos) {
    detail::common_string_view_type<T, CharT, Traits> sv = t;
    if (pos2 > sv.size())
      detail::throw_exception<std::out_of_range>("pos2 > t.size()");
    return replace(pos1, n1, sv.data() + pos2,
  ...
  */
  BOOST_STATIC_STRING_CPP14_CONSTEXPR
  basic_static_string& replace(size_type pos, size_type n1, size_type n2,
                               value_type c) {
...
#ifndef BOOST_STATIC_STRING_DOCS
  BOOST_STATIC_STRING_CPP14_CONSTEXPR
  basic_static_string& replace(const_iterator i1, const_iterator i2,
                               const basic_static_string& str) {
  ...
  */
  template <typename T
            ...
            >
  BOOST_STATIC_STRING_CPP14_CONSTEXPR basic_static_string& replace(
      const_iterator i1, const_iterator i2, const T& t) {
    detail::common_string_view_type<T, CharT, Traits> sv = t;
...
#endif
            >
  BOOST_STATIC_STRING_CPP14_CONSTEXPR size_type find(const T& t,
                                                     size_type pos = 0) const
#ifdef BOOST_STATIC_STRING_DOCS
      noexcept(
          detail::is_nothrow_convertible<const T&, string_view_type>::value)
#else
      noexcept(detail::is_nothrow_convertible<
               const T&,
               detail::common_string_view_type<T, CharT, Traits>>::value)
#endif
  {
    detail::common_string_view_type<T, CharT, Traits> sv = t;
...
*/
  ...
  find(const basic_static_string<M, CharT, Traits>& str,
  ...
  */
  template <std::size_t M>
  BOOST_STATIC_STRING_CPP14_CONSTEXPR size_type
  rfind(const basic_static_string<M, CharT, Traits>& str,
...
#endif
            >
  BOOST_STATIC_STRING_CPP14_CONSTEXPR bool starts_with(
...
#endif
            >
  BOOST_STATIC_STRING_CPP14_CONSTEXPR bool ends_with(
      T const& t) const noexcept {
    detail::common_string_view_type<T, CharT, Traits> sv = t;
    const size_type len = sv.size();
    return size() >= len &&
           !traits_type::compare(data() + (size() - len), sv.data(), len);
  }
 ...
 private:
  ...
  // data() + size() + 1.
  template <typename InputIterator>
  BOOST_STATIC_STRING_CPP14_CONSTEXPR size_type read_back(bool overwrite_null,
                                                          InputIterator first,
                                                          InputIterator last);

  BOOST_STATIC_STRING_CPP14_CONSTEXPR
  basic_static_string& replace_unchecked(size_type pos, size_type n1,
  ...
  BOOST_STATIC_STRING_CPP14_CONSTEXPR
  basic_static_string& replace_unchecked(const_iterator i1, const_iterator i2,
                                         const_pointer s, size_type n2);

  BOOST_STATIC_STRING_CPP14_CONSTEXPR
  basic_static_string& insert_unchecked(size_type index, const_pointer s,
                                        size_type count) {
    if (index > size())
      detail::throw_exception<std::out_of_range>("index > size()");
  ...
  BOOST_STATIC_STRING_CPP14_CONSTEXPR
  iterator insert_unchecked(const_iterator pos, const_pointer s,
                            size_type count);

  BOOST_STATIC_STRING_CPP14_CONSTEXPR
  basic_static_string& assign_unchecked(const_pointer s,
...
template <std::size_t N, typename CharT, typename Traits>
BOOST_STATIC_STRING_CPP14_CONSTEXPR inline bool operator==(
    const CharT* lhs, const basic_static_string<N, CharT, Traits>& rhs) {
  return detail::lexicographical_compare<CharT, Traits>(
...
template <std::size_t N, typename CharT, typename Traits>
BOOST_STATIC_STRING_CPP14_CONSTEXPR inline bool operator==(
    const basic_static_string<N, CharT, Traits>& lhs, const CharT* rhs) {
  return detail::lexicographical_compare<CharT, Traits>(
...
template <std::size_t N, typename CharT, typename Traits, class T
          ...
          >
BOOST_STATIC_STRING_CPP14_CONSTEXPR inline bool operator==(
    const T& lhs, const basic_static_string<N, CharT, Traits>& rhs) {
  detail::common_string_view_type<T, CharT, Traits> lhsv = lhs;
  return detail::lexicographical_compare<CharT, Traits>(
             lhsv.data(), lhsv.size(), rhs.data(), rhs.size()) == 0;
}
...
template <std::size_t N, typename CharT, typename Traits, class T
          ...
          >
BOOST_STATIC_STRING_CPP14_CONSTEXPR inline bool operator==(
    const basic_static_string<N, CharT, Traits>& lhs, const T& rhs) {
  detail::common_string_view_type<T, CharT, Traits> rhsv = rhs;
  return detail::lexicographical_compare<CharT, Traits>(
             lhs.data(), lhs.size(), rhsv.data(), rhsv.size()) == 0;
}
...
template <std::size_t N, std::size_t M, typename CharT, typename Traits>
BOOST_STATIC_STRING_CPP14_CONSTEXPR inline basic_static_string<N + M, CharT,
                                                               Traits>
...
template <std::size_t N, typename CharT, typename Traits>
BOOST_STATIC_STRING_CPP14_CONSTEXPR inline basic_static_string<N + 1, CharT,
                                                               Traits>
...
template <std::size_t N, typename CharT, typename Traits>
BOOST_STATIC_STRING_CPP14_CONSTEXPR inline void swap(
...
struct hash<
    ...
    > {
  std::size_t operator()(
      const boost::static_strings::basic_static_string<N, CharT, Traits>& str)
      const noexcept {
#if !defined(BOOST_STATIC_STRING_STANDALONE)
    return boost::hash_range(str.begin(), str.end());
#elif defined(BOOST_STATIC_STRING_HAS_ANY_STRING_VIEW)
    using view_type =
        typename boost::static_strings::basic_string_view<CharT, Traits>;
...
basic_static_string<N, CharT, Traits>::assign(size_type count, value_type ch)
    -> basic_static_string& {
  if (count > max_size())
...
template <std::size_t N, typename CharT, typename Traits>
BOOST_STATIC_STRING_CPP14_CONSTEXPR auto
basic_static_string<N, CharT, Traits>::assign(const_pointer s, size_type count)
    -> basic_static_string& {
  if (count > max_size())
    detail::throw_exception<std::length_error>("count > max_size()");
  this->set_size(count);
...
template <std::size_t N, typename CharT, typename Traits>
BOOST_STATIC_STRING_CPP14_CONSTEXPR auto
basic_static_string<N, CharT, Traits>::insert(const_iterator pos,
...
basic_static_string<N, CharT, Traits>::push_back(value_type ch) {
  const auto curr_size = size();
...
template <std::size_t N, typename CharT, typename Traits>
BOOST_STATIC_STRING_CPP14_CONSTEXPR auto
basic_static_string<N, CharT, Traits>::append(size_type count, value_type ch)
    -> basic_static_string& {
  const auto curr_size = size();
  if (count > max_size() - curr_size)
...
template <std::size_t N, typename CharT, typename Traits>
BOOST_STATIC_STRING_CPP14_CONSTEXPR auto
basic_static_string<N, CharT, Traits>::append(const_pointer s, size_type count)
    -> basic_static_string& {
  const auto curr_size = size();
  if (count > max_size() - curr_size)
    detail::throw_exception<std::length_error>("count > max_size() - size()");
...
basic_static_string<N, CharT, Traits>::resize(size_type n, value_type c) {
  if (n > max_size())
    detail::throw_exception<std::length_error>("n > max_size()");
  const auto curr_size = size();
  if (n > curr_size) traits_type::assign(data() + curr_size, n - curr_size, c);
  this->set_size(n);
  term();
}
...
template <std::size_t M>
BOOST_STATIC_STRING_CPP14_CONSTEXPR void
basic_static_string<N, CharT, Traits>::swap(
    basic_static_string<M, CharT, Traits>& s) {
  const auto curr_size = size();
...
template <std::size_t N, typename CharT, typename Traits>
BOOST_STATIC_STRING_CPP14_CONSTEXPR auto
basic_static_string<N, CharT, Traits>::replace(const_iterator i1,
                                               const_iterator i2, size_type n,
                                               value_type c)
...
basic_static_string<N, CharT, Traits>::replace(const_iterator i1,
  ...
  const bool inside =
      detail::ptr_in_range(curr_data, curr_data + curr_size, first_addr);
  if (inside && first_addr == i1 && n1 == n2) return *this;
...
template <std::size_t N, typename CharT, typename Traits>
BOOST_STATIC_STRING_CPP14_CONSTEXPR auto
basic_static_string<N, CharT, Traits>::find(const_pointer s, size_type pos,
                                            size_type n) const noexcept
    -> size_type {
  const auto curr_size = size();
  if (pos > curr_size || n > curr_size - pos) return npos;
  if (!n) return pos;
...
template <std::size_t N, typename CharT, typename Traits>
BOOST_STATIC_STRING_CPP14_CONSTEXPR auto
basic_static_string<N, CharT, Traits>::rfind(const_pointer s, size_type pos,
                                             size_type n) const noexcept
    -> size_type {
  const auto curr_size = size();
  const auto curr_data = data();
  if (curr_size < n) return npos;
  if (pos > curr_size - n) pos = curr_size - n;
  if (!n) return pos;
...
template <std::size_t N, typename CharT, typename Traits>
BOOST_STATIC_STRING_CPP14_CONSTEXPR auto
basic_static_string<N, CharT, Traits>::find_first_of(const_pointer s,
                                                     size_type pos,
                                                     size_type n) const noexcept
    -> size_type {
  const auto curr_data = data();
  if (pos >= size() || !n) return npos;
...
template <std::size_t N, typename CharT, typename Traits>
BOOST_STATIC_STRING_CPP14_CONSTEXPR auto
basic_static_string<N, CharT, Traits>::find_last_of(const_pointer s,
                                                    size_type pos,
                                                    size_type n) const noexcept
    -> size_type {
  const auto curr_size = size();
...
template <std::size_t N, typename CharT, typename Traits>
BOOST_STATIC_STRING_CPP14_CONSTEXPR auto
basic_static_string<N, CharT, Traits>::find_first_not_of(
...
template <std::size_t N, typename CharT, typename Traits>
BOOST_STATIC_STRING_CPP14_CONSTEXPR auto
basic_static_string<N, CharT, Traits>::find_last_not_of(
    const_pointer s, size_type pos, size_type n) const noexcept -> size_type {
  const auto curr_size = size();
  if (pos >= curr_size) pos = curr_size - 1;
  if (!n) return pos;
  pos = curr_size - (pos + 1);
...
template <typename InputIterator>
BOOST_STATIC_STRING_CPP14_CONSTEXPR auto
basic_static_string<N, CharT, Traits>::read_back(bool overwrite_null,
                                                 InputIterator first,
                                                 InputIterator last)
    -> size_type {
  const auto curr_data = data();
...
template <std::size_t N, typename CharT, typename Traits>
BOOST_STATIC_STRING_CPP14_CONSTEXPR auto
basic_static_string<N, CharT, Traits>::replace_unchecked(const_iterator i1,
                                                         const_iterator i2,
                                                         const_pointer s,
                                                         size_type n2)
    -> basic_static_string& {
...
template <std::size_t N, typename CharT, typename Traits>
BOOST_STATIC_STRING_CPP14_CONSTEXPR auto
basic_static_string<N, CharT, Traits>::insert_unchecked(const_iterator pos,
                                                        const_pointer s,
                                                        size_type count)
    -> iterator {
  const auto curr_data = data();
