...
class Graph {
 ...
 public:
  ...
  inline unsigned numNodes() const { return nNodes; }

  inline unsigned numEdges() const { return nEdges; }

  inline unsigned numArcs() const { return nArcs; }

  inline bool isDirected() const { return isDirected; }

  // get null value (standard is zero)
  virtual void setNullValue(const X& v) { nullVal = v; }

  // get null value (standard is zero)
  inline X getNullValue() const { return nullVal; }

  inline bool isNull(const X& x) const { return x == nullVal; }
 ...
 private:
  ...
  static bool compAsc(const pair<pair<unsigned, unsigned>, X>& v1,
                      const pair<pair<unsigned, unsigned>, X>& v2) {
    return v1.second < v2.second;
  }

  // comparison for sorting in descending order
  static bool compDesc(const pair<pair<unsigned, unsigned>, X>& v1,
                       const pair<pair<unsigned, unsigned>, X>& v2) {
    return v1.second > v2.second;
  }
