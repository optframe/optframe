...
class Command {
 protected:
  ...
  bool run_module(string mod, vector<Command<R, ADS, DS>*>& allCommands,
                  vector<PreprocessFunction<R, ADS, DS>*>& allFunctions,
                  HeuristicFactory<R, ADS, DS>& f,
    ...
    for (unsigned int i = 0; i < allCommands.size(); i++)
      if (allCommands[i]->canHandle(mod, input)) {
        ...
        return allCommands[i]->run(allCommands, allFunctions, f, dictionary,
                                   ldictionary, input, mod);
      }
 ...
 public:
  ...
  virtual bool canHandle(string module_name, string command_body) {
    ...
    for (unsigned i = 0; i < handles.size(); i++)
      if (handles[i] == module_name) return true;
  ...
  static bool validVariableName(string var) {
    if (var.length() == 0) {
      // cout << "Command::validVariableName error: empty variable!" <<
      // std::endl;
      return false;
    }
    ...
    if (number) {
      // cout << "Command::validVariableName error: cannot define a number! ('"
      // << var << "')" << std::endl; cout << "Did you forget a command
      // 'undefine variable' before this module?" << std::endl;
      return false;
    }
    ...
    if (optframe == "OptFrame") {
      // cout << "Command defineText: cannot define an OptFrame type! ('" << var
      // << "')" << std::endl; cout << "Did you forget a command 'undefine
      // variable' before this module?" << std::endl;
      return false;
    }
  ...
  static bool isInDictionary(string var,
  ...
  static bool pointVars(string new_var, string target_var,
                        std::map<std::string, std::string>& dictionary,
                        map<string, vector<string> >& ldictionary) {
    if (dictionary.count(target_var) > 0) {
      return defineText(new_var, dictionary.find(target_var)->second,
                        dictionary);
    } else if (ldictionary.count(target_var) > 0) {
      return defineList(new_var, ldictionary.find(target_var)->second,
                        ldictionary);
    } else {
      cout << "Command::pointVars error: variable '" << target_var
           << "' not found!" << std::endl;
      return false;
    }
  }

  static string solveTextVar(string variable,
                             std::map<std::string, std::string>& dictionary) {
    Scanner scanDef(variable);
    scanDef.useSeparators("$");
    if (!scanDef.hasNext()) return "";
    ...
    if (dictionary.count(var_name) == 0) {
      cout << "Command::solveReference error: variable '" << variable
  ...
  static bool define(string variable, string value,
                     std::map<std::string, std::string>& dictionary,
                     map<string, vector<string> >& ldictionary) {
    ...
    if ((trimValue == "") || (!scanner.hasNext())) return false;
    ...
    if ((next[0] == '[') || (ldictionary.count(trimValue) > 0)) {
      if (dictionary.count(variable) > 0) {
        cout << "Command::define error: variable '" << variable
             << "' already defined in text dictionary!" << std::endl;
        return false;
      }
  ...
  static bool defineText(string definition, string value,
                         map<string, string>& dictionary) {
    // will not need this check after better implementation of dictionary with
    // '$' before definitions
    // TODO

    if (!validVariableName(definition)) {
      cout << "Command::defineText error: not a variable name '" << definition
           << "'" << std::endl;
      return false;
    }
    ...
    if (definition[0] == '$') {
      // TODO: WHY?
      // cout << "defineText error: trying to define variable with dollar '" <<
      // definition << "'" << std::endl; return false;
      string newVar = solveTextVar(definition, dictionary);
      // cout << "newVar='" << newVar << "'" << std::endl;
      if (newVar == "") {
        cout << "defineText error: variable '" << definition << "' is empty!"
             << std::endl;
        return false;
      }
    ...
    while (scanner.hasNext()) {
      ...
      if (definition == next_word) {
        cout << "defineText: recursive definitions are not allowed! (define: '"
             << definition << "' as '" << next_word << "')" << std::endl;
        cout << "Did you forget a command 'undefine variable' before this "
                "module?"
             << std::endl;
        return false;
      }
  ...
  static bool defineList(string definition, vector<string>& list,
                         map<string, vector<string> >& ldictionary) {
    if (!validVariableName(definition)) {
      cout << "Command::defineList error: not a variable name '" << definition
           << "'" << std::endl;
      return false;
    }
    ...
    if (definition[0] == '$') {
      // TODO: WHY?
      cout << "defineList error: trying to define variable with dollar '"
           << definition << "' (can't use references for lists)" << std::endl;
      return false;
  ...
  static void undefineList(string definition,
                           map<string, vector<string> >& ldictionary) {
    ldictionary.erase(definition);
  }

  static void undefine(string definition, map<string, string>& dictionary,
                       map<string, vector<string> >& ldictionary) {
  ...
  bool testUnused(string id, Scanner& scanner) {
    if (scanner.hasNext()) {
      cout << "Error in module '" << id << "': unused text '" << scanner.rest()
           << "'" << std::endl;
      return false;
    }
  ...
  virtual bool run(std::vector<Command<R, ADS, DS>*>& allCommands,
                   ...
                   HeuristicFactory<R, ADS, DS>& factory,
                   map<string, string>& dictionary,
                   map<string, vector<string> >& ldictionary, string input) = 0;

  virtual string* preprocess(
      std::vector<PreprocessFunction<R, ADS, DS>*>& allFunctions,
      HeuristicFactory<R, ADS, DS>& hf, const map<string, string>& dictionary,
      const map<string, vector<string> >& ldictionary, string input) = 0;

  //{
  //	return defaultPreprocess(allFunctions, hf, dictionary, ldictionary,
  //input);
  //}

  static string* solveVars(const std::map<std::string, std::string>& dictionary,
                           const map<string, vector<string> >& ldictionary,
                           string input, string only_var = "") {
    // cout << "SOLVEVARS WITH '" << input << "' and only_var='" << only_var<<
    // "'" << std::endl;
    string result = "";
    ...
    for (unsigned i = 0; i < input.length(); i++) {
      ...
      if ((!inVar) && (var != "")) {
        ...
        if (dictionary.count(var) != 0)
          ...
          result.append(
              OptFrameList::listToString(ldictionary.find(var)->second));
        else {
          cout << "Error: variable '$" << var
               << "' not defined in any dictionary!" << std::endl;
      ...
      // capture var
      if ((!inVar) && (input[i] == '$')) inVar = true;

      // ordinary char
      if (!inVar) result += input[i];
    }
  ...
  static string* defaultPreprocess(
      std::vector<PreprocessFunction<R, ADS, DS>*>& allFunctions,
      HeuristicFactory<R, ADS, DS>& hf, const map<string, string>& dictionary,
      const map<string, vector<string> >& ldictionary, string input) {
    // stringstream input_func; // add spaces before and after '(', ')', '[',
    // ']', '{', '}', ';' and ','
    ...
    while (scanFunc.hasNext()) {
      ...
      if ((current == "(") && PreprocessFunction<R, ADS, DS>::functionExists(
                                  last, allFunctions))  // FUNCTION
      {
        pair<std::string, std::string>* p =
            PreprocessFunction<R, ADS, DS>::run_function(
                last, allFunctions, hf, dictionary, ldictionary,
                scanFunc.rest());
        ...
        if (p) {
          ...
          cout << "module preprocessing error in function '" << last << "'"
               << std::endl;
