...
#endif
class BuilderRVND : public LocalSearchBuilder<XES> {
 public:
  ~BuilderRVND() override = default;

  LocalSearch<XES>* build(Scanner& scanner, HeuristicFactory<XES>& hf,
                          std::string family = "") override {
    if (this->verbose) std::cout << "Debug: BuilderRVND::build()" << std::endl;
    ...
    return new RVND<XES>(eval, hlist, hf.getRandGen());
  }
  ...
  bool canBuild(std::string component) override {
    return component == RVND<XES>::idComponent();
  }

  static std::string idComponent() {
    std::stringstream ss;
    ss << LocalSearchBuilder<XES>::idComponent() << ":RVND";
    return ss.str();
  }
