...
class MultiObjILS : public MOILS, public MultiObjSearch<XMES> {
 ...
 public:
  ...
  MultiObjILS(sref<MultiEvaluator<XMES>> _mev, sref<InitialPareto<XMES>> _init_pareto, int _init_pop_size, sref<MOLocalSearch<XMES, XMEv>> _ls, sref<RandGen> _rg)
      //MultiObjILS(Evaluator<S>& _mev, InitialPareto<XMES>& _init_pareto, int _init_pop_size, MOLocalSearch<S, XEv>* _ls, RandGen& _rg)
      : init_pareto(_init_pareto), init_pop_size(_init_pop_size), ls(_ls), pMan(paretoManager<S, XMEv, XMES>(_mev)), rg(_rg) {
  }
