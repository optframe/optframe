...
public:
   GeneticAlgorithm(Evaluator<S>& _evaluator,
                    ...
                    unsigned populationMaxSize = 30,
                    unsigned numGenerations = 10,
                    double timelimit_ms = numeric_limits<double>::max()) //, Evaluation<DS> target)
