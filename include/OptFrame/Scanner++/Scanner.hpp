...
// TODO: ADJUST THIS OFFICIALLY TO CXX MODULES IN C++23 AS BELOW!
...
*/
  ...
  // move constructor
  File(File&& f) noexcept : filestream(f.filestream), filename(f.filename) {
  ...
  bool isOpen() noexcept { return filestream != nullptr; }

  int get() noexcept { return filestream && filestream->get(); }

  bool eof() noexcept { return filestream && filestream->eof(); }
...
MOD_EXPORT class Scanner final {
 private:
  ...
  void put_back(istream** input, string back) const {
    if ((*input)->eof()) {
      ...
      for (int i = ((int)back.length()) - 1; i >= 0; i--) {
        ...
        if ((*input)->fail()) {
          std::cout << "SCANNER ERROR PUTTING BACK CHAR '" << back[i] << "'"
                    << std::endl;
 ...
 public:
  ...
  void useDefaultSeparators() { useSeparators(string("\n\r\t ")); }
  ...
  void useSeparators(string s) { sep = s; }
 ...
 public:
  ...
  // returns true if file is open
  bool fileIsOpen() const noexcept { return inputfile && inputfile->isOpen(); }
 ...
 public:
  ...
  std::string peekNext() const noexcept {
    ...
    while (hasNextChar()) {
      ...
      getChar(
          *input,
          c);  // ignoring output (should be 'true', because of 'hasNextChar')
 ...
 public:
  static std::optional<int> parseInt(std::string s) {
    Scanner scanner(s);
    if (!scanner.hasNextInt()) return std::nullopt;
    return scanner.nextInt();
  }

  static std::optional<double> parseDouble(std::string s) {
    Scanner scanner(s);
    if (!scanner.hasNextDouble()) return std::nullopt;
 ...
 public:
  ...
  bool hasNext() const noexcept {
    ...
    char novo;
    if (!getChar(*input, novo)) return false;  // no next (and no exception!)
