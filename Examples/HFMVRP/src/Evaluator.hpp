   ...
   EvaluationHFMVRP evaluate(const SolutionHFMVRP& s) override
   {
      ...
      for (int v = 0; v < ((int)rep.size()); v++) {
         ...
         if (route.size() >= 3) // two depots
         {
            ...
            fo_dist_cost_q += pHFMVRP.getVehicleTypeDistCost(vehicleType) * routeDist[v];
   ...
   void exportEXCEL(const RepHFMVRP& rep, string outFile)
   {
      ...
      for (int v = 0; v < ((int)rep.size()); v++) {
         ...
         if (route.size() >= 3) {
            //nStores++;
            double sumDemands = 0;
            fprintf(f, "%d\t%d\t%d\t%f\n", v, v, pHFMVRP.getStoreID(route[1]), pHFMVRP.demands[route[1]]);
            sumDemands += pHFMVRP.demands[route[1]];
            for (int i = 2; i < (((int)route.size()) - 1); i++) {
               fprintf(f, "\t\t%d\t%f\n", pHFMVRP.getStoreID(route[i]), pHFMVRP.demands[route[i]]);
