...
private:
	...
	void chooseRoutes(vector<vector<int> >& newRep, vector<vector<int> >& parcialRep, vector<bool>& allocated, int vType)
	{
...
public:
	...
	ConstructiveSavings(ProblemInstance& _pHFMVRP, RandGen& _rg) : // If necessary, add more parameters
		pHFMVRP(_pHFMVRP), rg(_rg)
	{
	}
	...
	Solution<RepHFMVRP>& generateSolution()
	{
		...
		while (!feasibleSol)
		{
			...
			for (int types = 0; types < pHFMVRP.typesVehicles; types++)
			{
				...
				vTypeRep = savings(CriaLC(parcialRep, allocated, r), pHFMVRP.dist, &demands, pHFMVRP.getVehicleTypeCap(r));
			...
			cout << "number of clientes that are not allocated" << nonAllocated << endl;
			getchar();
			...
			cout << "number of clientes that STILL NOT allocated" << nonAllocated << endl;
			if (nonAllocated == 0)
			{
				feasibleSol = true;
				newRep = parcialRep;
