...
class S_TSPEvaluator : public Evaluator<RepSVRPDSP, AdsSVRPDSP, MySolution> {
 private:
 ...
 public:
  ...
  S_TSPEvaluator(ProblemInstance& _pSVRPDSP)
  ...
  Evaluation<> evaluate(const RepSVRPDSP& rep2,
    ...
    for (int i = 1; i < dlry_rep.size(); i++)
      if (dlry_rep[i] == 0)
        ...
        if (next > pSVRPDSP.n) next -= pSVRPDSP.n;
