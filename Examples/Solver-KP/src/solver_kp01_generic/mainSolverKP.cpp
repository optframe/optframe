...
class SolverKP01Generic {
 public:
  int runArgs(int argc, char** argv) {
    ...
    string local = std::string(span_args[0]);
    vector<string> params(argc);
    for (int i = 1; i < argc; i++) {
      params[i] = std::string(span_args[i]);
      if (params[i] == "--help") {
        std::cout << "help: solver_kp01_generic --config default_kp01.json"
                  << std::endl;
        return 0;
      }
      if (params[i] == "--info") {
    ...
    for (unsigned i = 0; i < params.size(); i++) {
      if (params[i] == "--config") config_file = params[i + 1];
    }

    if (config_file == "") {
      std::cout << "No run configuration. Try '--config', '--info' or "
                   "'--help'. Aborting!"
                << std::endl;
      return 1;
    } else {
