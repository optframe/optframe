...
public:
   ...
   DeltaMoveOrOptk(const super& s, ProblemInstance* _svrpdsp)
     : super(s.get_i(), s.get_j(), s.get_k(), _svrpdsp)
     , svrpdsp(_svrpdsp)
   {
      if (!svrpdsp) {
...
*/
   ...
   virtual Move<RepSVRPDSP, AdsSVRPDSP>* apply(RepSVRPDSP& rep, AdsSVRPDSP* _ads)
   {
      AdsSVRPDSP& ads = *_ads;
      // apply move
   ...
   op<Evaluation<>> cost(const Evaluation<>& e, const RepSVRPDSP& rep, const AdsSVRPDSP* ads, bool mayEstimate) override
   {
      return NULL;
   }
...
*/
      ...
      if (verbose)
         cout << endl
              << "END {" << nQ << "->" << f_inv << "}(" << e.getInfMeasure() << ")" << endl;
