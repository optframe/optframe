...
class ConstructiveGRandTSPOptimalSEQ
 ...
 public:
  ...
  ConstructiveGRandTSPOptimalSEQ(ProblemInstance& _pSVRPDSP, Scanner& _tsp,
          ...
          if ((q + pSVRPDSP.p[c]) <= pSVRPDSP.Q)  // pickup ok
            switch (option) {
              ...
              case 5:  // balance dist and pickup | (DIST - REV) / ( (cap - A) -
                ...
                cList.push_back(
                    make_pair(c, (pSVRPDSP.c(d, c) - pSVRPDSP.r[c]) /
                                     ((pSVRPDSP.Q - q) - pSVRPDSP.p[c])));
                break;
