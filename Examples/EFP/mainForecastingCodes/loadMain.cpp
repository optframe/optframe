...
int loadCompetitionBlind(int argc, char **argv)
{
	...
	for (int n = 0; n < nBatches; n++)
	{
		...
		for (int d = 6; d >= 0; d--)
		{
			...
			vector<double> dayValidation = rF.getPartsForecastsEndToBegin(tForecastDayByDay[d], 0, sol->first.getR().earliestInput);
			vector<vector<double> > validationBlindForecastings;
			validationBlindForecastings.push_back(dayValidation);

			dayBlindForecasts = pFC.returnBlind(sol, validationBlindForecastings);
...
//==========================================
	...
	for (int n = 0; n < nBatches; n++)
	{
		...
		for (int d = 6; d >= 0; d--)
		{
			...
			sol = pFC.run(timeES, timeVND, timeILS);
			vector<double> dayBlindForecasts;
			dayBlindForecasts = pFC.returnBlind(sol, validationBlindForecastingsDay);
