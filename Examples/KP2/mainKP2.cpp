...
int main(int argc, char** argv) {
  ...
  DEBUG {
    ...
    std::cerr << "Size: " << populationSize << " alpha: " << selectionRate
              << " omega: " << mutationRate << " num of gen: " << numGenerations
              << std::endl;
    std::cerr << "Constructing the optframe components that will be used."
              << std::endl;
  }
  ...
  optframe::GeneticAlgorithm ga(ev, ip, sl, el, cr, &mt, populationSize,
                                numGenerations);

  DEBUG std::cerr << "Starting Optframe GA main execution" << std::endl;

  std::pair<optframe::Solution<RepKP2>, optframe::Evaluation<>> best =
      ga.exec();

  DEBUG {
    std::cerr << "Optframe GA has finished with the following population: "
              << std::endl;
    int i = 0;
  ...
  std::cout << "Total Execution Time: " << ga.lastExecutionTime << " (s) "
            << std::endl;
