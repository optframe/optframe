...
class MoveST : public Move<RepOPM> {
 ...
 public:
  ...
  bool canBeApplied(const RepOPM& rep, const OPTFRAME_DEFAULT_ADS*) override {
    return (f1 != f2) && (c1 != c2) && (rep.second(f1, c1) > 0) &&
           (rep.first.at(f2) > -1) && (opm.isCompatible(c2, rep.first.at(f2)));
  }
  ...
  void print() const override {
    std::cout << "MoveST( from " << f1 << "," << c1 << " to " << f2 << "," << c2
         << ")" << std::endl;
  }
};
...
class NSEnumST : public NSEnum<RepOPM> {
 ...
 public:
  ...
  Move<RepOPM>* indexMove(unsigned int k) override {
    if (k > size()) {
      cerr << "Neighborhood ST Error! Move " << k
  ...
  unsigned int size() const override {
    return opm.getNumFrentes() * opm.getNumCaminhoes() * opm.getNumFrentes() *
           opm.getNumCaminhoes();
  }

  void print() { std::cout << "NSEnum ST (" << size() << ")" << std::endl; }
};
