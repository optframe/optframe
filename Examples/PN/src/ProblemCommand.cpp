...
bool
PNProblemCommand::load(string filename, HeuristicFactory<SolutionPN, EvaluationPN, ESolutionPN>& hf, map<string, string>& dictionary, map<string, vector<string>>& ldictionary)
{
   ...
   */
   ...
   //this->registerComponent(sref<PNEvaluator>{ new PNEvaluator(*p) }, "myEvaluator", hf, dictionary);
...
bool
PNProblemCommand::unload(HeuristicFactory<SolutionPN, EvaluationPN, ESolutionPN>& factory, map<string, string>& dictionary, map<string, vector<string>>& ldictionary)
{
   if (p)
      delete p;
   p = NULL;
