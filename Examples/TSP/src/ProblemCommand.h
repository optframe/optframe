...
class TSPProblemCommand {
 public:
  ...
  bool load(Scanner& scanner, HeuristicFactoryTSP& hf, map<string, string>& dictionary, map<string, vector<string>>& ldictionary) {
    ...
    hf.addComponent(nsseqtsp2opt, "OptFrame:NS:NSFind:NSSeq");  // no optimization
    ...
    //hf.addComponent(*new NSSeqTSPOrOpt<int, OPTFRAME_DEFAULT_ADS>, "OptFrame:NS:NSFind:NSSeq");
