...
public:
	...
	void createInstance(std::vector<pair<double, double> > data, string outputName)
	{
		ofstream output;
		...
		{
			output << setprecision(5) << data[indexData].second << std::endl;
		}
		output.close();
	}

	vector<pair<double, double> > separateThroughIntervals(std::vector<pair<double, double> > data, int intervalSeconds, string outputName, bool printOutput)
	{
...
//				getchar();
				...
				demandSeparated.push_back(std::make_pair(times, wattsAverage));

				if (printOutput)
					output << setprecision(10) << times << " " << setprecision(5) << wattsAverage << std::endl;

//				cout << wattsVector.size() << std::endl;
				wattsVector.clear();
//				cout << wattsVector << std::endl;
				timesPast = times;
				wattsVector.push_back(watts);
//				getchar();
			}
	...
	vector<pair<double, double> > interpolate(std::vector<pair<double, double> > data, int intervalSeconds)
	{
		...
		for (int i = 1; i < (int) data.size(); i++)
		{
			...
			if (timesPast != (times - 1))
			{
				for (int size = 1; size <= times - timesPast; size++)
				{
					...
					demandOutput.push_back(std::make_pair(timesPast + size, interpolation));
				}

			}
			else
				demandOutput.push_back(std::make_pair(times, watts));
...
//		{
//			cout << "Parameters File '" << inputName << "' not found" << std::endl;
//			exit(1);
//		}
