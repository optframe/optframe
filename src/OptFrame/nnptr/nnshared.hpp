...
template <class T> class NotNull {
public:
...
template <class T> auto make_NotNull(T &&t) noexcept {
...
namespace std {
template <class T> struct hash<nnptr::NotNull<T>> {
...
namespace nnptr {
//
template <typename T> class NNShared {
  using shared_type = T;
