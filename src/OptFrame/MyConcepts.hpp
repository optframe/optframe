...
//
template <class R> using sptr = std::shared_ptr<R>;
//
template <class R> using vsptr = std::vector<sptr<R>>;
...
template <optframe::comparability T> struct MyConceptsTestTotalOrder {};
...
// testing total order for some random classes
template <optframe::basic_arithmetics T> struct MyConceptsTestArithmetics {};
