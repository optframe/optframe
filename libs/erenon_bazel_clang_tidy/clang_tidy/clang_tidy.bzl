...
def _rule_sources(ctx):
    ...
    if hasattr(ctx.rule.attr, "srcs"):
        for src in ctx.rule.attr.srcs:
            #print("src=",src.files.to_list()[0].dirname)
            #x=src.files.to_list()[0].dirname
            #print(x.count("tests/thirdparty"))
            srcs += [_src for _src in src.files.to_list() if _src.is_source and _src.dirname.count("tests/thirdparty") <= 0]
    return srcs
